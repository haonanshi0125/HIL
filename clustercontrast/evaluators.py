...
def extract_cnn_feature(model, inputs,mode):
    ...
    # inputs1 = inputs
    # print(inputs)
    outputs,outputs_s = model(inputs,inputs,modal=mode)
...
def extract_features(model, data_loader, print_freq=50,flip=True,mode=0):
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, _, _) in enumerate(data_loader):
            ...
            outputs,outputs_s = extract_cnn_feature(model, imgs,mode)
            flip = fliplr(imgs)
            # print(flip)
            outputs_flip,outputs_flip_s = extract_cnn_feature(model, flip,mode)
