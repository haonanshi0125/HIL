...
class CM_D(autograd.Function):

    @staticmethod
    def forward(ctx, inputs, targets, features_d, momentum):
        ctx.features_d = features_d
        num_samples = features_d.shape[0]
        ctx.momentum = momentum
        ctx.save_for_backward(inputs, targets)
        ctx.features_d_c = ctx.features_d.view(-1, 768)
        outputs = inputs.mm(ctx.features_d_c.t())
        max_outputs, max_indices = torch.max(outputs.view(len(inputs), num_samples, 10), dim=2)
    ...
    def backward(ctx, grad_outputs):
        ...
        if ctx.needs_input_grad[0]:
            ...
            for i in range(len(inputs)):
                max_index = ctx.max_indices[i]
                grad_inputs[i] = grad_outputs[i].unsqueeze(0).mm(ctx.features_d[torch.arange(len(ctx.features_d)), max_index])

        # 对特征进行动量更新
        for i, (x, y) in enumerate(zip(inputs, targets)):
            # 计算输入 x 与特征中心 ctx.features_d[y] 的相似度
            similarities = x.unsqueeze(0).mm(ctx.features_d[y].t())  # 计算输入与所有中心的相似度
            max_similarity_index = torch.argmax(similarities)  # 找到最大相似度的索引

            # 只对与 x 最相似的中心进行更新
            ctx.features_d[y][max_similarity_index] = ctx.momentum * ctx.features_d[y][max_similarity_index] + (1. - ctx.momentum) * x
            ctx.features_d[y][max_similarity_index] /= ctx.features_d[y][max_similarity_index].norm()

        return grad_inputs, None, None, None

def cm_d(inputs, indexes, features_d, momentum=0.5):
    return CM_D.apply(inputs, indexes, features_d, torch.Tensor([momentum]).to(inputs.device))

class CM_D_r(autograd.Function):
...
def cm_d_r(inputs, indexes, features_d, momentum=0.5):
    return CM_D.apply(inputs, indexes, features_d, torch.Tensor([momentum]).to(inputs.device))
...
class ClusterMemory(nn.Module, ABC):
    def __init__(self, num_features, num_samples, temp=0.05, momentum=0.2, use_hard=False):
        ...
        self.register_buffer('features', torch.zeros(num_samples, num_features))
        self.register_buffer('features_d', torch.zeros(num_samples, 10, num_features))
    ...
    def forward(self, inputs, targets,ca=None,training_momentum=None):
        ...
        # outputs_d = cm_d(inputs, targets, self.features_d, self.momentum)
        outputs /= self.temp
        # outputs_d /= self.temp

        if ca == None:
            # loss = 0.5*F.cross_entropy(outputs, targets) + 0.5*F.cross_entropy(outputs_d, targets)
            loss = F.cross_entropy(outputs, targets)
            # loss = F.cross_entropy(outputs_d, targets)
        else:
            loss = (F.cross_entropy(outputs, targets,reduction='none')*ca).mean()

        return loss

class ClusterMemory_shared(nn.Module, ABC):
    def __init__(self, num_features, num_samples, temp=0.05, momentum=0.2, use_hard=False):
        super(ClusterMemory_shared, self).__init__()
        self.num_features = num_features
        self.num_samples = num_samples
        self.momentum = momentum
        self.temp = temp
...
class ClusterMemory_r(nn.Module, ABC):
    def __init__(self, num_features, num_samples, temp=0.05, momentum=0.2, use_hard=False):
        super(ClusterMemory_r, self).__init__()
        self.num_features = num_features
        self.num_samples = num_samples
        self.momentum = momentum
        self.temp = temp
        self.use_hard = use_hard
        self.register_buffer('features', torch.zeros(num_samples, num_features))
        self.register_buffer('features_d', torch.zeros(num_samples, 3, num_features))
...
class ClusterMemory_shared_r(nn.Module, ABC):
    def __init__(self, num_features, num_samples, temp=0.05, momentum=0.2, use_hard=False):
        super(ClusterMemory_shared_r, self).__init__()
        self.num_features = num_features
        self.num_samples = num_samples
        self.momentum = momentum
        self.temp = temp
